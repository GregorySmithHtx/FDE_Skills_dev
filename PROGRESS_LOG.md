...
**Pillar 6 — a real schema catch before anything got built, then a further simplification, then correct generalization of an edge case.** Claude scoped the `TimelineItem` route as a plain insert needing only `term_id`. Greg caught this was wrong first: `TimelineItem` also carried its own `evidence_id`, and `Evidence` itself needs a `source_term_id` plus one of `term_id`/`relation_id`/`chain_id` — a 3-table fan-out, not a single insert. He then went further, unprompted: `TimelineItem` doesn't need `evidence_id` at all, since the event it points at (e.g. "Discovery of Uranus") is its own term, and evidence/relations belong on that term directly like any other term — a `TimelineItem` row is just a date/type placement. Pushed on a fair follow-up (what if the date itself is disputed between sources?), he generalized correctly to the schema's existing pattern rather than inventing new structure: a disputed date becomes sibling terms under a shared `derivation_of` base, each with its own evidence and its own `TimelineItem` row.

**Pillar 4 — the exact gap flagged by session 20's pillar-9 correction, closed.** Session 20's Alembic setup was entirely Claude's hands, and Greg's correction at the time was "let me do new things at least once." This session, offered a first attempt on a real schema change (dropping `timeline_items.evidence_id`), he took it: didn't remember the mechanics, asked directly ("do I just change the model file and save, or do I need to tell Alembic what to look for"), got a straight four-step answer, then ran the whole sequence himself — hit and self-corrected a `script_location` error (needed to run from the repo root where `alembic.ini` lives, not inside the package directory), reviewed the generated migration correctly (confirmed it drops the FK before the column), applied it, and understood the rollback story (`downgrade()` reverses one revision at a time) without needing it explained twice.

**A real, well-reasoned architecture direction, translating his own domain expertise, not adopting Claude's first suggestion.** Named the actual problem with `__init__.py`'s helper layer: `search_legacy_terms_by_name` and its three siblings each fuse "search" and "create" into one function, so assembling a multi-part object means bouncing between similarly-named functions that secretly also write — real, specific code-reading, not a vague complaint. Asked what this pattern is usually called; Claude's first guess (SQLAlchemy ORM event hooks, i.e. implicit `before_insert` triggers) was a reasonable technical parallel but wrong for his actual mental model, and he corrected it precisely: Skuid's model-driven forms aren't automatic triggers, they're a browser-side picklist/combobox search against a model, filled out to viability, submitted once — explicit existence-checks before creation, not implicit magic after the fact. That correction is worth noting on its own: he didn't just accept an available technical-sounding answer, he checked it against his own real domain knowledge and rejected the mismatch. From there, concluded unprompted that testing this dependency-resolution by hand (raw API calls, no UI) is itself the friction, and floated starting the React frontend now rather than continuing to script around it.

**Honest caveats.** Session ended before either open fork got resolved (consolidate the helper layer vs. start React) and before an actual `TimelineItem` row got created end-to-end — Greg stopped for sleep mid-decision, not at a natural stopping point. The Evidence CRUD routes he added independently mid-session repeat the same fused search+insert shape being critiqued, a live example he can use next session rather than a hypothetical.

**Roadmap status**: `timeline_items` schema simplified and migrated (evidence_id dropped), `GET`/`POST` routes written, `GET` verified live. Full technical detail: `symbol_constellation/log/2026-09-02.md`.

---

### 2026-09-01 (session 22) — FastAPI paralysis from session 21 resolved: a full search/dedup/migration flow, self-diagnosed bugs, and a real legacy data-quality catch

Same-day continuation of session 21, picking back up on FastAPI after a break. Session 21 had flagged a real struggle getting started — worth being precise about what kind: freezing on day one of a framework he'd never touched before (FastAPI) is ordinary first-exposure difficulty, not the same thing as the 2026-08-17 "blank-page paralysis" note, which described a more general trouble starting from scratch even in territory he already knows. Pillar 6/9 judgment stayed intact underneath either way. This session is a partial test of that distinction — the syntax gap closed considerably faster than session 21's pace suggested, consistent with "new tool, first exposure" rather than a deeper generalized block.

**Pillar 4 — real, isolated reps, most self-diagnosed once the concept was named, not before.** Built `legacy_search.py` standalone (plain `sqlite3`, tested via CLI) before wiring it into FastAPI — his own instinct once offered the choice, matching the "isolate the new library from the new framework" pattern that worked well last session. Real bugs, each one his own: passing a bare string instead of a `(value,)` tuple to `sqlite3`'s param-binding argument; writing `?` inside an f-string `{}` (not valid — `?` isn't a Python expression); `Path("~/...")` not expanding `~`, since shell tilde-expansion happens before Python ever sees a CLI argument, but a literal `~` typed into source code gets no such treatment. All three fixed correctly on the first explanation, no repeat of the same mistake. A subtler one caught the same way: comparing a plain `enum.Enum` member to a raw string (`category == "person"`) is always `False` — needs `str`-mixed enum or comparison against the enum member itself — which had silently broken his own dedup branch; found via review, fixed correctly once explained. Sharpest self-inflicted bug: a FastAPI route function named identically to an imported helper (`search_legacy_sources`) silently shadowed the import at module-load time, so a helper that called what it thought was the import ended up calling the route function instead — real infinite recursion, correctly traced back once the shadowing mechanism was explained (Python resolves names at call time, not definition time).

**Pillar 6 — the dedup rule itself was his own design, not coached.** Given the shape of the problem (search a legacy SQLite graph before creating a duplicate term in the new Postgres schema), Greg specified the actual rule himself: no legacy match → create fresh; legacy match(es) found → check each by `legacy_term_id`, only import what's missing, never create a plain duplicate. Verified live against real data (searching "Plato" twice produced the same 5 rows, zero duplicates) — this is real evidence for pillar 6 landing in application-logic design, not just schema design where it's mostly been demonstrated before.

**Pillar 7/9 — a real data-quality catch, initiated by his own skepticism, not prompted.** When asked to cross-reference his own manual `category='source'` tagging pass against the `sources` table, Greg pushed back on a first check-query Claude ran (comparing `sources.title` to `terms.name` directly) — correctly recognizing those two fields were deliberately designed to differ (full bibliographic string vs. glossary-safe shorthand, a distinction from session 19), so an "exact match" cross-check was structurally guaranteed to produce false negatives. The better check (`terms.source_id` → `sources.source_id`, threading through the actual foreign key) was his own proposal, and it surfaced a real, previously-unknown data bug: term 5423 ("Dogme et Rituel de la Haute Magie") had a `source_id` pointing at the wrong book's `sources` row (Waite's *Pictorial Key to the Tarot* instead of Lévi's own work), likely from citation context bleeding into ingestion. Fixed directly in `constellation.db`, then had the pairing re-verified across all 22 rows before proceeding — matching the standing "verify before reassuring" discipline, but here it was Greg driving the verification, not just accepting a Claude-run check.

**Honest caveats.** The overall shape of the session (search-in-isolation-first, one-concept-at-a-time explanations) continues to be a collaboration pattern Claude adjusts to on Greg's explicit correction, not something Greg has needed to ask for twice this session — a good sign the session-21 pacing correction actually landed. Verification against live Postgres/sqlite state after each claimed-working step was mostly Claude's initiative (per [[feedback_verify_before_reassuring]]), except the source-pairing cross-check, which was Greg's own idea start to finish.

**Roadmap status**: `terms`/`sources` now have real, working FastAPI routes with verified dedup against live data (22 source terms + their bibliographic rows migrated in). `timeline_items` routes not started. Full technical detail: `symbol_constellation/TIMELINE_ITEM_DESIGN.md` (updated same session) and `symbol_constellation/log/2026-09-01.md`.
